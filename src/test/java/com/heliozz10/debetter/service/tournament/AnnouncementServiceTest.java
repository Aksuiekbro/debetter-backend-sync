...
class AnnouncementServiceTest {
    ...
    @Test
    void createAnnouncementsWithTheSameImageExtensionUsesDistinctStorageUrls() {
        ...
        verify(fileService, times(2)).uploadImage(any(), eq("announcements"), anyString());
    }

    @Test
    void updateAnnouncementUsesANewStorageUrlAndCleansUpOnlyTheReplacedImageAfterCommit() {
        Tournament tournament = new Tournament();
        tournament.setId(53L);
        OrganizerProfile editor = new OrganizerProfile();
        Url originalImage = new Url();
        originalImage.setId(101L);
        originalImage.setUrl("/uploads/announcements/53.jpg");
        ...
        doAnswer(invocation -> {
            assertSame(
                    ...
                    "Reference checks must run before orphan removal can flush the old URL row"
            );
            return null;
        ...
        verify(fileService).uploadImage(eq(replacement), eq("announcements"), anyString());
        verify(fileService).deletePhysicalFileAfterCommit(originalImage);
    }

    @Test
    void removeAnnouncementUsesTournamentThenAnnouncementIdsAndCleansUpItsImageAfterCommit() {
        Url image = new Url();
        image.setId(101L);
        image.setUrl("/uploads/images/announcements/53-unique.jpg");
        Announcement announcement = new Announcement();
        announcement.setId(11L);
        announcement.setImageUrl(image);
        ...
        verify(fileService).deletePhysicalFileAfterCommit(image);
